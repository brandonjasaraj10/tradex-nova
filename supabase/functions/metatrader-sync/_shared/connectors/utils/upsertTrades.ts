...
export async function upsertTrades(
...
): Promise<{ inserted: number; updated: number; skipped: number; errors: any[]; dedupe_conflicts: number; }> {
  if (trades.length === 0) {
    return { inserted: 0, updated: 0, skipped: 0, errors: [], dedupe_conflicts: 0 };
  }
  ...
  let updated = 0;
  let skipped = 0;
  let dedupe_conflicts = 0;

  for (const trade of trades) {
    try {
      if (!trade.broker_trade_id || !trade.user_id || !trade.broker_connection_id) {
        skipped++;
        continue;
      }
  ...
  return { inserted, updated, skipped, errors, dedupe_conflicts };
}

export async function updateConnectionStats(
  supabase: SupabaseClient,
  connectionId: string,
  stats: { trades_synced?: number; cursor?: string; success: boolean; error?: string; }
): Promise<void> {
  ...
  await supabase.from('user_broker_connections').update(updates).eq('id', connectionId);
}